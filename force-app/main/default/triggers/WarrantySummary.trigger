trigger WarrantySummary on Case (before insert) {
    ...
    for (Case myCase : Trigger.new){
        // Set up variables to use in the summary fields
        String purchaseDate         = myCase.Product_Purchase_Date__c.format();
        String createdDate          = DateTime.now().format();
        Decimal warrantyDays        = myCase.Product_Total_Warranty_Days__c.intValue();
        ...
        myCase.Warranty_Summary__c = 'Product purchased on' + purchaseDate 
                                   ...
                                   + 'Warranty is for ' + warrantyDays + ' days and is '
                                   + warrantyPercentage + '% through its warranty period.\n'
